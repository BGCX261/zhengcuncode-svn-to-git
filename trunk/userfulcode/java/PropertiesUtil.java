...
public class PropertiesUtil extends ArrayList {
	...
	public PropertiesUtil(String fName, String encoding) {
		try {
			fileName = PropertiesUtil.class.getResource(fName).getPath();
			this.setCharacterEncoding(encoding);
			if (!isURIFileExist(fileName))
				this.write("");
	...
	private static boolean isURIFileExist(String fileName) {
	...
	public static String read(String fileName, String encoding)
		...
		try {
			while ((s = in.readLine()) != null) {
				// 为什么转换了反而会是乱码
				sb.append(s);// new String(s.getBytes("iso8859_1"),
				// encoding));
				sb.append("\n");
			}
		} finally {
